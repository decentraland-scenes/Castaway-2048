...
function openChest() {
  const state = boardWrapper.getComponent(OpenLerp)
...
const spawner = {
  ...
  spawnGem(val: number, x: number, y: number) {
    ...
    const shapeIndex = gemValues.indexOf(val)
    ent.addComponentOrReplace(gemModels[shapeIndex])

    const t = ent.getComponentOrCreate(Transform)
    t.scale.setAll(0.5)
    t.position = gridToScene(x, y)

    const td = ent.getComponentOrCreate(GemData)
    td.reset(val, x, y)
...
function addRandomGem() {
  ...
  for (const gem in gems.entities) {
    ...
    const gemPos = gemData.nextPos.x + gemData.nextPos.y * 4
    const index = emptyCells.indexOf(gemPos)
...
function shiftBlocks(direction: Directions) {
  ...
  for (let row = 0; row < board.size; ++row) {
    ...
    for (const gem in gems.entities) {
      const gemPos = gems.entities[gem].getComponent(GemData).nextPos
      let pos: number
      switch (direction) {
        case Directions.UP:
          if (gemPos.x === row) {
    ...
    for (let target = 0; target < board.size; ++target) {
      ...
      let blocked = false
      for (let tile = target + 1; tile < currentRow.length; ++tile) {
        if (currentRow[tile]) {
          const gemData = currentRow[tile].getComponent(GemData)
          ...
          if (currentRow[target] === null || alreadyMerged === true) {
            ...
            const targetData = currentRow[target].getComponent(GemData)
            if (
...
const chestLightAnimator = new Animator()
...
for (let i = 0; i < gemValues.length; i++) {
  const gemMod = new GLTFShape('models/' + gemVal + '.glb')
...
const swipeChecker = new SwipeDetection()
